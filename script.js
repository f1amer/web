...
function normalChat(q){const s=q.toLowerCase().trim();const clean=s.replace(/[^a-z0-9\s']/g,'').trim();
 ...
 if(/\b(bye|goodbye|see you|see ya)\b/.test(s))return"Goodbye! Thanks for visiting, and have a great day.";
 if(s.includes('what can you do')||s.includes('what do you know'))return"I can chat normally, tell you about Shishir's cybersecurity background, do basic calculations, check current weather by city, and help prepare an email.";
 return null}
